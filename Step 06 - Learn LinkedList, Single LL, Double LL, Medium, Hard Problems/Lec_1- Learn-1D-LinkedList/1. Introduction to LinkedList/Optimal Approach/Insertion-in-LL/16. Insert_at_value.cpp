...
Node *insertAfterValue(Node *head, int targetValue, int newValue)
{
    ...
    if (head == nullptr)
    {
        cout << "The list is empty. Cannot insert after " << targetValue << "." << endl;
        return head;
    }
    ...
    // Create a new node and insert it after the target node
...
Node *insertBeforeValue(Node *head, int targetValue, int newValue)
{
    ...
    if (head == nullptr)
    {
        cout << "The list is empty. Cannot insert before " << targetValue << "." << endl;
        return head;
    }

    // Case 1: If the target value is at the head
    ...
    // Case 2: If the target value is not found
    ...
    // Case 3: Insert before the target value
    Node *newNode = new Node(newValue);
    newNode->next = temp->next;
    temp->next = newNode;
